...
class Trajectory_Dataloader():
    def __init__(self, args):
        ...
        # [修改 2] 预加载所有场景的背景图
        self.scene_images = {} # 存储 {set_index: image_tensor}
        if self.args.dataset == 'eth5':
            ...
            # 手动映射 data_dirs 索引到 DATASET_NAME_TO_NUM
    ...
    def _load_scene_images(self):
        """加载并预处理所有场景图片"""
        print("正在加载场景图片...")
        for idx, dir_path in enumerate(self.data_dirs):
            img_path = os.path.join(dir_path, 'reference.png')
            if not os.path.exists(img_path):
                # 尝试找其他常见的名字，或者使用全黑图代替
            ...
            self.scene_images[idx] = img_tensor
        print("场景图片加载完成。")
    ...
    def get_train_batch(self, idx):
        ...
        if scene_img is None:
            scene_img = torch.zeros(3, 224, 224) # fallback
